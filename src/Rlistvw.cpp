...
// insert columns
	...
	CSize sz[4] =
	 {
	   dc.GetTextExtent( CString(_gszColumnLabel[0]) ),
	   dc.GetTextExtent( CString(_gszColumnLabel[1]) ),
	   dc.GetTextExtent( CString("С мечеными устр.") ),
	   dc.GetTextExtent( CString("С мечеными устр.") )
	 };
...
TDiplomDoc* CRowListView::GetDocument() // non-debug version is inline
{
...
LRESULT CRowListView::OnCommandHelp( WPARAM wp, LPARAM lParam )
 {   
   return IDH_EditorSchemes;
 }

LRESULT CRowListView::OnCommandHelpF1( WPARAM wp, LPARAM lParam )
 {   
